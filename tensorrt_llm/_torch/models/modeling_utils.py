...
class DecoderModelForCausalLM(nn.Module,
    ...
    def __init__(self, model: TModel, *, config: ModelConfig[TConfig],
        ...
        if hasattr(config.pretrained_config, 'tie_word_embeddings') and config.pretrained_config.tie_word_embeddings:
...
def _load_weights_impl(model: Union[nn.Module, DecoderModelForCausalLM],
    ...
    def load_single_module(name, module):
        ...
        if len(module._parameters) > 0:
            ...
            if hasattr(model.config, 'tie_word_embeddings') and model.config.tie_word_embeddings and name.startswith("lm_head"):
                return
